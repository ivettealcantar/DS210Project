...
pub fn plot_degree_centrality(degree_centrality: &[(String, usize)]) -> Result<(), Box<dyn std::error::Error>> {
    let root = BitMapBackend::new("output/degree_centrality_chart.png", (1600, 1200)).into_drawing_area();
    root.fill(&WHITE)?;
...
pub fn plot_rates(records: &[CleanRecord]) -> Result<(), Box<dyn Error>> {
    let root = BitMapBackend::new("output/rates.png", (800, 600)).into_drawing_area();
    root.fill(&WHITE)?;
...
pub fn plot_national_averages(records: &[CleanRecord]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let root = BitMapBackend::new("output/national_averages.png", (800, 600)).into_drawing_area();
    root.fill(&WHITE)?;
    ...
    println!("Plot saved as 'output/national_averages.png'.");
    Ok(())
}
pub fn plot_trends_over_time(records: &[CleanRecord], state: Option<&str>) -> Result<(), Box<dyn Error>> {
    ...
    let file_name = if let Some(state_name) = state {
        format!("output/{}_trends_over_time.png", state_name.to_lowercase())
    } else {
        "output/nationwide_trends_over_time.png".to_string()
    };
    ...
    pub fn plot_crime_rates_comparison(records: &[CleanRecord], states: &[&str]) -> Result<(), Box<dyn Error>> {
        ...
        // Set up the chart
        let file_name = "output/az_mass_crime_rates_comparison.png";
        let root = BitMapBackend::new(&file_name, (1200, 800)).into_drawing_area();
        root.fill(&WHITE)?;
